...
async function renderDecks() {
  try {
    ...
    const list = window.getElement('decks-list');
    if (!list) return;
    
    if (!decks.length) {
      list.innerHTML = `<div class="empty-state">
        <div class="empty-state-icon">🎴</div>
        <div class="empty-state-text">Нет колод</div>
        <div class="empty-state-sub">Добавьте карточки из библиотеки или загрузите JSON файл</div>
      </div>`;
      return;
    }
    ...
    for (const deck of decks) {
      ...
      el.innerHTML = `
        ...
        <div class="deck-info">
          ...
          <div class="deck-meta">${total} ${isMatch ? 'блоков' : 'карточек'}</div>
        </div>
    ...
    const list = window.getElement('decks-list');
    if (list) {
      list.innerHTML = '<div class="empty-state"><div class="empty-state-icon">⚠️</div><div class="empty-state-text">Ошибка загрузки данных</div></div>';
    }
  }
}
...
async function startCardStudy(deckId) {
  try {
    ...
    if (window.showToast) window.showToast('⚠️ Ошибка загрузки колоды');
  }
}
...
function showCard() {
  ...
  if (progressNum) progressNum.textContent = studyIdx + 1;
  ...
  const stats = window.getElement('card-stats');
...
function updateSessionStats() {
  const correctEl = window.getElement('stat-correct');
  const wrongEl = window.getElement('stat-wrong');
  const rateEl = window.getElement('stat-rate');
  
  if (correctEl) correctEl.textContent = sessionKnown;
  if (wrongEl) wrongEl.textContent = sessionErrors;
...
function showCompletionScreen() {
  ...
  const compCorrect = window.getElement('comp-correct');
  const compWrong = window.getElement('comp-wrong');
  const compRate = window.getElement('comp-rate');
  
  if (compCorrect) compCorrect.textContent = sessionKnown;
  if (compWrong) compWrong.textContent = sessionErrors;
  
  const total = sessionKnown + sessionErrors;
  const rate = total > 0 ? Math.round((sessionKnown / total) * 100) : 0;
  if (compRate) compRate.textContent = rate + '%';
}
...
async function startMatch(deckId) {
  try {
    ...
    const titleEl = window.getElement('match-deck-name');
    if (titleEl) titleEl.textContent = deck.name;
    
    window.showScreen('match-screen');
    renderMatchSet();
  } catch (error) {
    console.error('Ошибка запуска сопоставления:', error);
    if (window.showToast) window.showToast('⚠️ Ошибка загрузки');
  }
}

function renderMatchSet() {
  if (matchSetIdx >= matchSets.length) {
    showMatchCompletion();
    return;
  }
  ...
  const grid = window.getElement('match-grid');
  if (!grid) return;
  ...
  pairs.forEach((pair, idx) => {
    ...
    chip.dataset.side = 'left';
    chip.onclick = () => selectChip('left', idx, chip);
  ...
  rightItems.forEach((pair, displayIdx) => {
    const originalIdx = pairs.indexOf(pair);
    const chip = document.createElement('div');
    chip.className = 'match-chip';
    chip.textContent = pair.example || '';
    chip.dataset.displayIdx = displayIdx;
    chip.dataset.originalIdx = originalIdx;
    chip.dataset.side = 'right';
    chip.onclick = () => selectChip('right', displayIdx, chip, originalIdx);
...
function selectChip(side, idx, chip, originalIdx) {
  ...
  if (side === 'left') {
    if (matchSelected.left === idx) {
      matchSelected.left = null;
      return;
    }
    matchSelected.left = idx;
    chip.classList.add('selected');
  } else {
    if (matchSelected.right === idx) {
      matchSelected.right = null;
      matchSelected.rightOriginal = null;
      return;
    }
    matchSelected.right = idx;
...
function connectPair() {
  matchConnections.push({
    left: matchSelected.left,
    rightDisplay: matchSelected.right,
...
function drawLines() {
  const canvas = window.getElement('match-canvas');
  const grid = window.getElement('match-grid');
  ...
  matchConnections.forEach(conn => {
    ...
    line.setAttribute('stroke', '#9575CD');
...
function restartMatch() {
  matchSetIdx = 0;
  matchSets = matchSets.sort(() => Math.random() - 0.5);
  renderMatchSet();
}
